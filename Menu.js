...
* https://github.com/Shploop/Blooket-Cheats/blob/main/LICENSE
*/
...
(() => {
    const cheat = (async () => {
        ...
        const guiWrapper = createElement("div", {
            ...
            (gui = createElement("div", {
                ...
                createElement("div", {
                    ...
                    innerHTML: "GitHub - Shploop",
                    onclick: () => window.open("https://github.com/Shploop/Blooket-Cheats", "_blank").focus()
                }),
        ...
        const Cheats = {
            ...
            brawl: [
                {
                    name: "Multiply Enemy XP",
                    description: "Multiplies enemy XP drop value",
                    run: function () {
                        for (const collider of Object.values((function react(r = document.querySelector("body>div")) { return Object.values(r)[1]?.children?.[0]?._owner.stateNode ? r : react(r.querySelector(":scope>div")) })())[1].children[0]._owner.stateNode.state.game.scene.physics.world.colliders._active.filter(x => x.callbackContext?.toString().includes('dmgCd'))) {
                            ...
                            enemies.children.entries.forEach(e => e.val *= 5);
                        }
                    }
                },
                {
                    name: "Very Slow Enemy Speed",
                    description: "Makes enemies move a lot slower",
                    run: function () {
                        for (const collider of Object.values((function react(r = document.querySelector("body>div")) { return Object.values(r)[1]?.children?.[0]?._owner.stateNode ? r : react(r.querySelector(":scope>div")) })())[1].children[0]._owner.stateNode.state.game.scene.physics.world.colliders._active.filter(x => x.callbackContext?.toString().includes('dmgCd'))) {
                            const enemies = collider.object2;
                            let _start = enemies.classType.prototype.start;
                            enemies.classType.prototype.start = function () { _start.apply(this, arguments), this.speed *= .5 }
                            enemies.children.entries.forEach(e => e.speed *= .1);
                        }
                    }
                },
                {
    ...
    let img = new Image;
    img.src = "https://raw.githubusercontent.com/Shploop/Blooket-Cheats/main/autoupdate/timestamps/gui.png?" + Date.now();
